...
print "chi2 (matching baryon density) = ", chi2_best(YoYSPSfit=YoYSPSfit_best)
...
#filename = 'rho-cluster-scalar0.01'
#filename = 'rho-cluster-scalar0.0005'
#filename = 'rho-cluster-scalar0.0075'
filename = 'rho-cluster-vector'

#alphalab = r'$\alpha\prime = 0.01$'
#alphalab = r'$\alpha\prime = 0.0005$'
#alphalab = r'$\alpha\prime = 0.0075$'
alphalab = r'$m_V = $ '



# NFW (Jim - scalar) #
infile_dat = dirname + filename + '.dat'
logr_sc = np.loadtxt(fname=infile_dat, delimiter=' ', usecols = 0)
logrho_sc = np.loadtxt(fname=infile_dat, delimiter=' ', usecols = 1)
...
# SCALAR #
# scalar - Jim #
veldispLOS_sc = veldispLOS(logr=logr_sc, logrho=logrho_sc, Rmin=binsL, Rmax=binsR)



## chi^2 test (neglecting horizontal error bar for simplicity) ##
# for the reduced chi^2, just divide the result by 5, which is the number of the data points #
chi2_SIDM_cmp = chi2(obs=veldispLOS_SIDM_cmp, exp=proj_stdv_data_exp, dobs=None, dexp=dproj_stdv_data_exp) # num point = len(r_exp_tot)
print "\nchi2_SIDM_cmp = ", chi2_SIDM_cmp


veldispLOS_sc_cmp = veldispLOS(logr=logr_sc, logrho=logrho_sc, Rmin=dcr_data_exp_l, Rmax=dcr_data_exp_r)
chi2_sc_cmp = chi2(obs=veldispLOS_sc_cmp, exp=proj_stdv_data_exp, dobs=None, dexp=dproj_stdv_data_exp) # num point = len(r_exp_tot)
print "chi2_sc_cmp = ", chi2_sc_cmp
...
# VECTOR #
text1_sc = r'A 2537 -- Model 1'
text2_sc = r'$m_X = $ MeV' + '\n' + r'$\alpha\prime = $ ' + '\n' + r'$\delta m = {0:s}$'.format(as_si(1.0e-30, 0)) + r' eV'


fig5 = plt.figure(num=filename, figsize=(10, 7), dpi=100)
...
ax5.plot(mid_bins, veldispLOS_sc, color ='blue', linestyle ='-.', lw=2.0, label=alphalab)
...
fig5.tight_layout()
fig5.show()
#fig5.savefig(dirname + 'fig_' + filename + '.pdf', dpi=200, bbox_inches='tight', pad_inches=0.08)
