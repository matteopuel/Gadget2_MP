...
def rhoH(roa, M, a) :
	return M / (2.0 * np.pi * a**3) / roa / (1.0 + roa)**3
...
str_out = "test2/scattering/data/" + fn + ".dat"
...
Nscatt_bins = [0 for j in range(0, len(bins)-1)]
...
ratioN_bins = [float(Nscatt_bins[j]) / float(Npart_mean_bins[j]) for j in range(0, len(bins)-1)]
...
dNscatt_bins = [np.sqrt(el) for el in Nscatt_bins]
...
dratioN_bins = [ratioN_bins[j] * np.sqrt( (dNscatt_bins[j]/Nscatt_bins[j])**2 + (dNpart_mean_bins[j]/Npart_mean_bins[j])**2 ) for j in range(0, len(bins)-1)]
...
## save the information for each value of h_S ##
