...
// MPUEL: add variables for CosmicTimeTable
#ifdef ADM_MODEL_ON
  double initial = 0;
#endif
...
// MPUEL: add integration from a = 0 to a_in
#ifdef ADM_MODEL_ON
  F.function = &cosmictime_integ;
  gsl_integration_qag(&F, 0, All.TimeBegin, 0,
    1.0e-10, WORKSIZE, GSL_INTEG_GAUSS41, workspace, &result, &abserr);
  initial = result;

  if(ThisTask == 0)
    printf("t (a = 0 till a = a_in) = %f\n", initial);
  
// MPUEL: add integration from a = a_in to a_end
  for(i = 0; i < COSMIC_TIME_TABLE_LENGTH; i++)
  {
      F.function = &cosmictime_integ;
      gsl_integration_qag(&F, All.TimeBegin, All.TimeBegin + (All.TimeMax - All.TimeBegin) / COSMIC_TIME_TABLE_LENGTH * (i + 1), 0,
        1.0e-10, WORKSIZE, GSL_INTEG_GAUSS41, workspace, &result, &abserr);
      CosmicTimeTable[i] = initial + result;
  }
...
double get_drift_factor(int time0, int time1)
{
  ...
  if(i1 <= 1)
    ...
    df1 = DriftTable[i1 - 1] + (DriftTable[i1] - DriftTable[i1 - 1]) * (u1 - i1); // interpolate
...
// MPUEL: for cosmictime_integ(), see adm_model.c
