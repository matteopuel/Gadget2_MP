...
// MPUEL: add All.BunchSizeADM
#ifdef ADM_MODEL_ON
#if defined(ANNIHILATION_DM) || defined(SCATTERING_DM)

  All.BunchSizeADM =
    (All.BufferSize * 1024 * 1024) / (2 * sizeof(struct admdata_in) + 2 * sizeof(struct admdata_out));

  ADMDataIn = (struct admdata_in *) CommBuffer;
  ADMDataGet = ADMDataIn + All.BunchSizeADM;
  ADMDataResult = (struct admdata_out *) (ADMDataGet + All.BunchSizeADM);
  ADMDataPartialResult = ADMDataResult + All.BunchSizeADM;
...
#ifdef ADM_MODEL_ON
#if defined(ANNIHILATION_DM) || defined(SCATTERING_DM)
      printf("Communication buffer has room for %d particles in annihilation/scattering\n", All.BunchSizeADM);
#endif
#endif
