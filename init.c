...
// MPUEL: initialize All.PreviousTime
  All.PreviousTime = 0;
...
// MPUEL: initialize All.N_scatterings and All.N_annihilations
// I define them here and not in read_ic.c as other variables because the following ones are not read from snapshots or restart files (it should not matter though)
#ifdef ANNIHILATION_DM
    All.N_annihilations = 0;
    All.N_annihilations_in_timestep = 0;
    All.N_annihilations_tot = 0;
#endif

#ifdef SCATTERING_DM
    All.N_scatterings = 0;
    All.N_scatterings_in_timestep = 0;
    All.N_scatterings_tot = 0;
#endif
...
// MPUEL: initialize AnnihilateFlag and ScatterFlag to all particles (same reason as before)
#ifdef ANNIHILATION_DM
         P[i].AnnihilateFlag = 0; 
#endif
#ifdef SCATTERING_DM
         P[i].ScatterFlag = 0; 
#endif      

    } /* end block started from line 94 */
...
// MPUEL: in the case we use MAKEGLASS option, we need to initialize ADM variables
#ifdef ADM_MODEL_ON
    for(i = 0; i < NumPart; i++)
    {
