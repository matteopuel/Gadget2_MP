...
int NumADMUpdate;        /*!< number of active ADM particles on local processor in current timestep  */
#endif
#endif
...
#ifdef ADM_MODEL_ON
gsl_rng *random_generator_ADM; /*!< random number generator for ADM calculations*/
#endif
...
// MPUEL: add CosmicTimeTable
#ifdef ADM_MODEL_ON
double CosmicTimeTable[COSMIC_TIME_TABLE_LENGTH];  /*!< table for cosmic time */
#endif
...
// MPUEL: add ADMData structs useful for annihilation/scattering
#ifdef ADM_MODEL_ON
#if defined(ANNIHILATION_DM) || defined(SCATTERING_DM)
