...
int main(int argc, char **argv)
{
  ...
  MPI_Init(&argc, &argv);
  MPI_Comm_rank(MPI_COMM_WORLD, &ThisTask); // local processor
  MPI_Comm_size(MPI_COMM_WORLD, &NTask);  // other processors
  ...
  if(argc < 2) // because the starting command is: mmpirun -np (arg=0) 32 (arg=1) ./... (arg=2)
    {
      if(ThisTask == 0)
	{
  ...
  strcpy(ParameterFile, argv[1]); // equivalent to: ParameterFile = argv[1] (copy the name of the parameter file)

  if(argc >= 3)
    RestartFlag = atoi(argv[2]); // 1 (or 2) to restart the simulation from restart (or snapshot) file
  else
    RestartFlag = 0;
