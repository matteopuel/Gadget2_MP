...
// MPUEL: add new two/three blocks in the snapshot
#ifdef ADM_MODEL_ON

    case IO_ANGOSC:   /* initial oscillation angle */
#ifdef OSCILLATION_DM
      for(n = 0; n < pc; pindex++)
  if(P[pindex].Type == type)
    {
      *fp++ = P[pindex].OscillationAngle;
      n++;
    }
#endif
      break;
  ...
  if(P[pindex].Type == type)
    ...
    } // close block starting from line 171
...
// MPUEL: add new two/three blocks in the snapshot (their presence is controlled by blockpresent())
#ifdef ADM_MODEL_ON
...
int get_datatype_in_block(enum iofields blocknr)
{
  ...
  switch (blocknr)
    ...
    default: // MPUEL: I do not need to add the two/three new blocks here because they are float as the default type
...
// MPUEL: add two/three new blocks in the snapshot (their presence is controlled by blockpresent())
#ifdef ADM_MODEL_ON
    case IO_ANGOSC:   /* oscillation angle */
    case IO_DTSCAT:   /* dt_scatter */
      for(i = 0; i < 6; i++)
...
// MPUEL: add two/three new blocks in the snapshot
#ifdef ADM_MODEL_ON

#ifndef OSCILLATION_DM
    if(blocknr == IO_ANGOSC)
      return 0;
#endif

#ifndef OUTPUTDT_SCATTER
    if(blocknr == IO_DTSCAT)
      return 0;
#endif
...
#ifdef ADM_MODEL_ON
      case IO_ANGOSC:
  strncpy(Tab_IO_Labels[IO_ANGOSC], "ANGO", 4);
  break;
      case IO_DTSCAT:
  strncpy(Tab_IO_Labels[IO_DTSCAT], "DTSC", 4);
  break;
#endif
...
#ifdef ADM_MODEL_ON
    case IO_ANGOSC:
      strcpy(buf, "OscillationAngle");
      break;
    case IO_DTSCAT:
      strcpy(buf, "ScatteringTimestep");
      break;
#endif
...
#define SKIP  {my_fwrite(&blksize,sizeof(int),1,fd);}
  ...
  if(All.ComovingIntegrationOn) // All.Time = a (scale factor)
  ...
  for(blocknr = 0; blocknr < IO_NBLOCKS; blocknr++)
	...
	} /* close block starting from line 933 */
    }
