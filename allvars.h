...
#include <gsl/gsl_math.h> // MPUEL: add <gsl/gsl_math.h> library for M_PI
#include "tags.h"
...
// MPUEL: add electronvolt definition in cgs units (g cm^2 / s^2 = erg) and other constants
#define  EV                1.602176634e-12
#ifdef ADM_MODEL_ON
#define  ADM_type          1 /* Particle Type that can oscillate, scatter and annihilate, i.e. Halo particles! */
#define  annihilated_type  5 /* Particle Type of particles that have annihiliated */
#endif // end of ADM_MODEL_ON
...
// MPUEL: add max length for importing tables
#ifdef VELOCITY_ANGULAR_DEPENDENT_CROSS_SECTION
#define MAXLEN_THETA_TABLE 1000000 /*!< maxmimum number of thetas in 2D theta table */
#define MAXLEN_CROSSSECTION_TABLE 10000 /*!< maxmimum number of cross-sections in cross-section table */
#endif
...
// MPUEL: add number of active ADM particles
#ifdef ADM_MODEL_ON
#define COSMIC_TIME_TABLE_LENGTH  100000   /*!< length of the lookup table used to hold the cosmic time */ 

#if defined(ANNIHILATION_DM) || defined(SCATTERING_DM)
extern int NumADMUpdate;        /*!< number of active ADM particles on local processor in current timestep  */
#endif
...
// MPUEL: number generator for oscillation, scattering and annihilation
#ifdef ADM_MODEL_ON
extern gsl_rng *random_generator_ADM; /*!< random number generator for ADM calculations*/
#endif
...
// MPUEL: add additional output file for tests or other informations
#ifdef ADM_MODEL_ON
extern FILE *FdAdmModel;
extern double CosmicTimeTable[COSMIC_TIME_TABLE_LENGTH];  /*!< table for cosmic time */  
#endif
...
extern struct global_data_all_processes
{
  ...
  // MPUEL: add number of ADM particles fitting into the communication buffer
#ifdef ADM_MODEL_ON
#if defined(ANNIHILATION_DM) || defined(SCATTERING_DM)
  int BunchSizeADM;         /*!< number of particles fitting into the communication buffer in the annihilation/scattering computation */
#endif
#endif
...
// MPUEL: add PreviousTime
  double PreviousTime; /*!< previous time of the simulation */
  ...
  // MPUEL: ADM model parameters
#ifdef ADM_MODEL_ON
  double c; /*!< speed of light in code units */
#endif // end ADM_MODEL_ON

#if defined(VECTOR_MEDIATOR) || defined(SCALAR_MEDIATOR)
  double MediatorDMmassratio; /*!< Mass ratio between mediator and DM */
  double VectorOrScalarCoupling;  /*!< Coupling between vector or scalar mediator and DM */
#endif

#ifdef OSCILLATION_DM
  double MajoranaMass;  /*!< Majorana mass term in units of eV */
  double delta_m; /*!< Majorana mass term in code units */
#endif

#ifdef ANNIHILATION_DM
  unsigned long N_annihilations; /*!< Number of DM annihilations computed at this Task */
  unsigned long N_annihilations_in_timestep;  /*!< Sum N_annihiltions over all Tasks in each timestep. Only the root Task keeps track of this value */
  unsigned long N_annihilations_tot;  /*!< Sum N_annihiltions over all Tasks in simulation. Only the root Task keeps track of this value */
  double AnnihilationCrossSectionSwave; /*!< Cross-section in units of cm^3/g/sec */
  double sigmav_s;  /*!< Cross-section in code units */
  double AnnihilateSearchRadius;  /*!< Comoving annihilate search radius */
  double AnnihilateSearchRadiusMaxPhys; /*!< Maximum Physical annihilate search radius */
#endif

#ifdef SCATTERING_DM
  unsigned long N_scatterings; /*!< Number of DM scatterings computed at this Task */
  unsigned long N_scatterings_in_timestep;  /*!< Sum N_scatterings over all Tasks in each timestep. Only the root Task keeps track of this value */
  unsigned long N_scatterings_tot;  /*!< Sum N_scatterings over all Tasks in simulation. Only the root Task keeps track of this value */
  double ScatteringCrossSection;  /*!< Cross-section in units of cm^2/g */
  double sigma_scatter; /*!< Cross-section in code units */
  double ScatterSearchRadius; /*!< Comoving scatter search radius */
  double ScatterSearchRadiusMaxPhys;  /*!< Maximum Physical scatter search radius */

  double ProbabilityTol;  /*!< Max prbability for scattering DM in order to introduce a timestep delimiter */
#endif

#ifdef VELOCITY_ANGULAR_DEPENDENT_CROSS_SECTION
  char ThetaTableFilename[200];
  double ThetaTable[MAXLEN_THETA_TABLE];
  char ThetaTableFlag[MAXLEN_THETA_TABLE];
  int ThetaTableLength;
  int Nv; /*!< number of velocities for which theta table is calculated */
  int Ntheta; /*!< number of thetas in theta table for each velocity */
  double vmin; /*!< minimum velocity for which theta table is calculated */
  double vmax; /*!< maximum velocity for which theta table is calculated */
  char CrossSectionTableFilename[200];
  double CrossSectionTable[MAXLEN_CROSSSECTION_TABLE];
  char CrossSectionTableFlag[MAXLEN_CROSSSECTION_TABLE];
  int CrossSectionTableLength;
#endif
...
extern struct particle_data
{
  ...
  int Ti_endstep;               /*!< marks end of current timestep of particle on integer timeline */ 
...
// MPUEL: add initial oscillation angle, oscillation probability and timestep delimiter for scattering DM particles
#ifdef OSCILLATION_DM
  double OscillationAngle;   /*!< Oscillation angle associated to each DM particle */
#endif

#ifdef ANNIHILATION_DM
  short int AnnihilateFlag;   /*!< particle flag for particles that have annihilated during the timestep */
#endif

#ifdef SCATTERING_DM
  double dt_scatter;    /*!< timestep for single particle (see KODA's code dt_sidm);
                             just for scattering because in annihilation particles disappear */
  short int ScatterFlag;   /*!< particle flag for particles that have scattered during the timestep */
#endif
...
// MPUEL: add 2 to the number of blocks for ADM_MODEL_ON
#ifdef ADM_MODEL_ON
...
// MPUEL: add three new blocks in the snapshot (their presence is controlled by blockpresent() in io.c)
#ifdef ADM_MODEL_ON
  IO_ANGOSC,
  IO_DTSCAT,
#endif
...
// MPUEL: add ADMData structs useful for ADM annihilation/scattering
#ifdef ADM_MODEL_ON
#if defined(ANNIHILATION_DM) || defined(SCATTERING_DM)

extern struct admdata_in
{
  FLOAT Pos[3];
  FLOAT Vel[3];
  int Index;
  int Task;
  int Ti_begstep;               /*!< marks start of particle timestep on integer timeline */
  int Ti_endstep;               /*!< marks end of particle timestep on integer timeline */ 
  int Core2;
 ...
 *ADMDataIn,                   /*!< holds particle data for DM annihilation/scattering to be exported to other processors */
 *ADMDataGet;                  /*!< holds imported particle data for DM annihilation/scattering */


extern struct admdata_out
{
  FLOAT DeltaVel[3];

#ifdef OSCILLATION_DM // "Delta" means difference: new minus old values (for safety) 
  double DeltaOscillationAngle;   /*!< oscillation angle associated to each DM particle */
#endif
#ifdef ANNIHILATION_DM // "Delta" means difference: new minus old values (for safety) 
  short int DeltaAnnihilateFlag;   /*!< particle flag for particles that have annihilated during the timestep */
#endif
#ifdef SCATTERING_DM
  double Deltadt_scatter;
  short int DeltaScatterFlag;   /*!< particle flag for particles that have scattered during the timestep */
#endif
}
 *ADMDataResult,               /*!< stores the locally computed DM annihilation/scattering results for imported particles */
 *ADMDataPartialResult;        /*!< imported partial DM annihilation/scattering results from other processors */

#endif // end ANNIHILATION_DM and SCATTERING_DM
#endif // end ADM_MODEL_ON
