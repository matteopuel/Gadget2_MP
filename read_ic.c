 ...
 *  to this value assuming a mean molecular weight either corresponding to
...
// MPUEL: initialize dt_scatter and OscillationAngle to zero for all particles
// MPUEL: This is done only initially, namely when RestartFlag == 0. Otherwise, the following parameters are read from the restart or snapshot files
#ifdef ADM_MODEL_ON
		for(i = 0; i < NumPart; i++)
		{
    ...
    } /* close block starting from line 115 */
...
// MPUEL: add two/three new blocks in the snapshot, which are needed to define the IC (they presence is set by blockpresent())
#ifdef ADM_MODEL_ON

    case IO_ANGOSC:		/* oscillation angle */
#ifdef OSCILLATION_DM
      for(n = 0; n < pc; n++)
	P[offset + n].OscillationAngle = *fp++;
#endif
      break;

    case IO_DTSCAT:		/* dt_scatter */
#if defined(SCATTERING_DM) && defined(OUTPUTDT_SCATTER)
      for(n = 0; n < pc; n++)
	P[offset + n].dt_scatter = *fp++;
#endif
      break;
...
#define SKIP2  {my_fread(&blksize2,sizeof(int),1,fd);}
  ...
  if(ThisTask == readTask)
    {
      if(All.ICFormat == 1 || All.ICFormat == 2) /* read the header */
	{
	  if(!(fd = fopen(fname, "r")))
	    {
...
#ifdef HAVE_HDF5
      if(All.ICFormat == 3) /* read the header */
	{
    ...
    } // close block starting from line 288
  else
  ...
  if(All.TotNumPart == 0)
	...
	All.MassTable[i] = header.mass[i];
      ...
      if(RestartFlag == 2) // read from snapshot file
  ...
  for(blocknr = 0; blocknr < IO_NBLOCKS; blocknr++)
    {
      if(blockpresent(blocknr)) // if blocknr!=0
	{
	  ...
	  npart = get_particles_in_block(blocknr, &typelist[0]); // from the header
	...
	} /* close block starting from line 425 */
    } /* close block starting from line 423 */
  ...
  for(type = 0; type < 6; type++)
	...
	n_for_this_task++;
      ...
      NumPart += n_for_this_task; // NumPart is determined by the header.npart in each processor
